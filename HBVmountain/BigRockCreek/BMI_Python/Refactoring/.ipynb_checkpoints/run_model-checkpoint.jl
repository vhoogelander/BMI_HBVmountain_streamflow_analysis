...
#     tmax::Int128 = length(Precipitation[:,1])
    ...
    Discharge::Float64 = 0.0
    WBtotal::Float64 = 0.0
    Snow_Extend::Array{Float64,1} = zeros(Total_Elevationbands)
    ...
    Evaporation_Mean_Current = Evaporation_Mean
    ...
    Riparian_Discharge::Float64, Total_Discharge::Float64, Total_Interception_Evaporation::Float64, Total_Soil_Evaporation::Float64, bare_storage::Storages, forest_storage::Storages, grass_storage::Storages, rip_storage::Storages, Slowstorage::Float64, WB::Float64, Total_Prec::Float64 = allHRU(bare_input, forest_input, grass_input, rip_input, bare_storage, forest_storage, grass_storage, rip_storage,
      bare_parameters, forest_parameters, grass_parameters, rip_parameters,         Slowstorage, slow_parameters)
    ...
    Discharge = Total_Discharge   #Total_Discharge/1000 * Area / (3600 * 24) # mm convert it to meter and than * area / seconds in one day
        # OPTIONAL: store the total amount of snow in each elevation (considering whole catchment)
        Snow_Elevations, Snow_Extend = snowperelevation(bare_input, forest_input, grass_input, rip_input, bare_storage, forest_storage, grass_storage, rip_storage, Total_Elevationbands)
...
function run_model_glacier(Area, Evaporation_Mean::Float64, Glacier::Array{Float64,1}, Precipitation::Array{Float64,1}, Temp::Array{Float64,1},
        ...
        slow_parameters::Slow_Paramters, Total_Elevationbands, Elevation_Percentage)
    # the function takes as input the parameters of each HRU, the inital storage values of each HRU, the inital value of the slow storage
    # KS, ratio riparian, all inputs
