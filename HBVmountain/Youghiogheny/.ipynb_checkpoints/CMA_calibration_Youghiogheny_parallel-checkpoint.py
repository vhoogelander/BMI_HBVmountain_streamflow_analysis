...
def run_model_cma(parameters):
    ...
    forcing = nc.Dataset('Data/Youghiogheny/HBVmountain_ERA5_Youghiogheny_1986_2005.nc') #Catchment dependent
...
def full_calibration(ntimes):
    PARAMETERS_BOUNDS = [[-2.0, 2.0],
                     ...
                     [0.05, 0.5]]
    area = 347.1 #km2
    observation = pd.read_csv('Data/Youghiogheny/Discharge_Youghiogheny.csv', index_col=0).streamflow / (area * 1e6) * 1000 *86400
    ob_list = []
    params_list = []
    results_doc_list = []
...
def main():
    ...
    if rank == 0:
        ...
        name = 'paramsets_youghiogheny.csv'
        outdir = './output_youghiogheny'
