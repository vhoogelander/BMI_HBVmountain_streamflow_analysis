...
def wrapped_objective_function(scaled_parameters, observation, iterations):
...
def cma_calibration(parameter_bounds, observation, MAXITER):
...
def full_calibration(ntimes, iterations):
    PARAMETERS_BOUNDS = [[-2.0, 2.0],
                     [1.0, 5.0],
                     [0.001, 1.0],
                     [0.1, 0.9],
    ...
    for n in range(ntimes):
        full_output = cma_calibration(PARAMETERS_BOUNDS, observation, iterations)
